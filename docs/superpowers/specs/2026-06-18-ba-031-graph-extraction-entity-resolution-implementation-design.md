...
The Python worker (`ennam.kg.python/src/ennam_kg/queue/consumer.py`) reads **only Redis `BRPOP`** (FIFO, mirrors Go `LPUSH` in `internal/queue/publisher.go`). The DB-backed `query_queue` (migration 029, `store/queue.go`) is **AI-query-coupled** (`ai_query_id UUID NOT NULL REFERENCES ai_queries(id)`) and **not wired to the Python worker** — it cannot be reused as the extraction substrate.
...
Python BRPOP   → Pass2: for each new entity:
                   POST /embeddings (symmetric passage: prefix) → Go upserts embedding →
...
| **OQ-003** | **`canonical_name + description`**, with a **symmetric `passage:` prefix mandated on both stored and query embeddings**. Re-embed on merge only when the embedded `content_hash` changes (BR-006.4). | `e5` is prefix-instructed (`encode_query`/`encode_passage` in `local_model.py`). For entity-vs-entity blocking, a `query:`/`passage:` mismatch silently degrades cosine and masquerades as a threshold problem. Both sides must use `passage:`. |
...
|---|---|---|---|---|
| **R1** | **Blocking recall** — a pair missed at cosine blocking is invisible to the verifier *forever*; NFR-257 dies silently. Worse than a wrong-merge (which is at least downstream-detectable). | Med-High | Critical | Symmetric `passage:` prefix; start threshold low (0.72–0.75); K=10; **recall ≥90% @K=10 gate at 8b** on real VN name variants |
