...
| Gate 1 edge-whitelist + node-type enforcement (already exists; **register new types**) | Go | `internal/validation/edge_whitelist.go` (existing); node-type validation already enforced in `service/node.go:508` against config.yaml; register the closed types in `config/types.go` (`ValidNodeTypes`) + `config.yaml` (`node_types` schemas + `edge_whitelist`) + DB CHECK |
...
| Embedding **vector generation** (local) + **persist** | Python→Go | Python computes the vector locally via `embeddings/local_model.py` (`encode_query`/`encode_passage`, lines 59/63), then persists via the existing `upsert_node_embeddings` client → Go `POST /api/v1/projects/{id}/node-embeddings/batch` (`handler/document.go:157` `BatchUpsertEmbeddings`) → `NodeEmbeddingStore.Upsert`. **There is no generic `/api/v1/embeddings` endpoint** — vectors are computed in-process in Python (mirrors `decompose.py:249/260`). |
...
Python BRPOP   → Pass2: for each new entity:
                   compute vector locally (symmetric prefix) → upsert_node_embeddings → Go BatchUpsertEmbeddings →
...
- **FR-NEW-5 — Register the closed vocabulary in the app-layer schema (corrected).** *Correction after code verification:* node creation is **already** app-validated — `service/node.go:508-519` (`validateStoreRequest`) rejects unknown node types against the **config.yaml `node_types` schemas** (the single source of truth), and `config.ValidNodeTypes` already exists (`config/types.go:57`). The earlier "only edges are validated" premise was wrong. The real gap: the OQ-001 work must register the closed types in **three** places, not two — (1) the DB `node_type` CHECK, (2) the `edge_whitelist`, and **(3) the config.yaml `node_types` schemas + `config.ValidNodeTypes`**. Without (3), `validateStoreRequest` rejects every `person`/`organization`/… node *before* it reaches the DB. Each new `node_types` entry must define `display_name`, `description`, `required`, and `fields` (e.g. `canonical_name`, `description`, `subtype`) — and must reconcile with Gate-2 completeness validation (required-field enforcement). This is the corrected scope of the OQ-001 deliverable, not a new validator.
...
- **Unit (Go):** edge-whitelist + node-type validation (both reject out-of-vocab; drop-don't-coerce; assert the closed types are accepted once registered at all 3 surfaces); merge transaction (re-point, UNIQUE-collision supersede-not-delete, provenance union with source-edge identity); optimistic-lock conflict + retry; `merged_into` chain-follow to `active` only + cycle guard (fail loud); provenance cap (NFR-267).
...
8. **Closed vocabulary registered at all 3 enforcement surfaces** (DB CHECK + config.yaml `node_types` schemas + `edge_whitelist`) — node-type app validation already exists, so registration (not a new validator) is the work.
