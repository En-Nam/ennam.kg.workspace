...
- **Merge re-points edges (why B IMPROVES, not degrades, relatedness):** `merge.go` Step 5 "re-point the member's edges to the canonical" (`RepointedEdges`, `RepointEdgesTx`). So when a member concept is merged into the canonical, its `document→member` mentions become `document→canonical` — the canonical accumulates the members' document mentions, and Step-2's `merged_into=''` filter then sees ONE node mentioned by all those documents (more shared-entity links, not fewer). Empirically confirmed: the 1b-merged canonical "ubnd tỉnh trà vinh" is mentioned by 65 documents (accumulated from merged members). Step-2 IDF is query-time, so it self-updates after merges.
- **One Apply call covers the band:** `Apply` uses `maxSuggestions = 10000` (`apply_suggestions.go:85`), so a single `/apply` processes all 4695.
- **Reuse:** `internal/service/merge.go` (reversible merge: `merged_into`, `merge_undo`, alias union, `re_embed_pending`, edge re-point), `internal/service/apply_suggestions.go` (`HubMergeManifest` dry-run shape to mirror for the manifest), `internal/store/merge_suggestion.go` (`ListByProject`, `UpdateDecision`). `ennam.kg.python/.../resolution/pass2.py` (`route_pairs` — the verdict source; NO changes).
